...
function initListners(days, matrix) {
    var daysIndexes = days.map(function (value) {
    ...
    $('#mini-cloud button').click(function () {
        toggleResize()
    });
    ...
    $('.tic').click(function (e) {
        ...
        setLabel([start.format('D/MM/YYYY'), end.format('D/MM/YYYY')]);
...
function resetStyleTags() {
    // reset first of all
    reset();
}

function setStyleTags(highlights) {
    ...
    if (typeof highlights === 'undefined' || !highlights || highlights === '') {
        ...
        $('.tag').each(function (e) {
            let id = this.id;
            // console.log(id,highlights.indexOf(id),highlights);
            if (highlights.indexOf(id) < 0) {
                $(this).removeClass('highlight');
                $(this).addClass('downlight');
            } else {
...
function setTag(tagID, tag) {
    ...
    let days = dataSource.reduce((r, e) => {
        if (e.text.toLowerCase() === tag.toLowerCase()) {
            r.push({value: e.size, key: moment(e.day).format('d/MM/YYYY'), day: e.day, tag});
        }
        return r;
    }, []);
    ...
    setLabel(tag);
    // highglight days
    addStyleDates(days.map(e => e.day));
    // highlight tag
    setStyleTags([tagID]);
}
...
function setLabel(label = '') {
    ...
    if (Array.isArray(label)) {
        label = 'From <span class="date">' + label[0] + '</span>  to <span class="date">' + label[1] + '</span>';
    } else if (typeof label === 'string') {
        label = 'Tag: <span class="tag">' + label + '</span>';
    }

    // append label
    $('#interval').show();
    $('#interval label').empty();
    $('#interval label').append(label);
}
